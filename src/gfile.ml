...
let printOutArc oc g id_o = 
  let rec loop acu = function 
    | [] -> fprintf oc "%s%!" acu
    | x :: rest -> match x with 
      | (id_d,x) -> loop ((stringArc id_o id_d x)^acu) rest
  in 
  loop "" (out_arcs g id_o)

let export g path = 
  let ff = open_out path in 
  if not (node_exists g 0) then fprintf ff "%s%!" stringBase
  else 
    fprintf ff "%s%!" stringBase ;
  n_iter_sorted g (printOutArc ff g) ;
  fprintf ff "}" 