...
let stringBase = "digraph finite_state_machine {
	rankdir=LR;
  size=\"8,5\" ";;
  
let stringArc idO idD = "LR_"^string_of_int(idO)^" -> LR_"^string_of_int(idD)^" ;";;
let rec parcoursOutArcs idO acu = function
  |[]->acu
  |(id,x)::rest->  parcoursOutArcs idO (acu^(stringArc idO id)) rest 
;;

let printId = fun id -> Printf.printf " ID : %d%n%!" id;;

let export g = 
  if node_exists g 0 then stringBase^"}" 
  else 
    n_iter 
    ...
    let rec parcoursGraphe acu = function
    |[]->acu
    |(id,l)::rest -> parcoursGraphe (parcoursOutArcs id acu l) rest
    in
    parcoursGraphe stringBase g
;;


(*Utiliser les fonctions de graph.mli au lieu de 
parcours comme si c'etait une liste*)